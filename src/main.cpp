...
int main() {
  ...
  // thresholds for distance to cars ahead and behind for classifying them as being too close
  double too_close_ahead_dist = 28;
  double too_close_behind_dist = 4;

  h.onMessage(
      [&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&lane,&ref_vel, &too_close_ahead_dist, &too_close_behind_dist](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
          uWS::OpCode opCode) {
          ...
          if (s != "") {
            ...
            if (event == "telemetry") {
              ...
              for (int i=0; i<sensor_fusion.size(); i++)
              {
                ...
                else if (car_s - check_car_s < too_close_ahead_dist)  // other car is behind and too close
                {
              ...
              {
                // if it's possible change to the lane to the left
                if ((lane>0)&&(not car_ahead_too_close_in_left_lane)&&(not car_behind_too_close_in_left_lane))
                {
                  lane--;
                }
                // if it's possible change to the lane to the right
              ...
              else
              {
                ...
                // change to the middle lane if possible, so that there are more choices in future
                if (((lane==0)&&(not car_ahead_too_close_in_right_lane)&&(not car_behind_too_close_in_right_lane))
                    ||((lane==2)&&(not car_ahead_too_close_in_left_lane)&&(not car_behind_too_close_in_left_lane)))
                {
                  lane = 1;
                }
              }
              ...
              // transform to local coordinates so that reference angle is always 0
              ...
              //define the actual (x,y) points to be used by the planner
              vector<double> next_x_vals;
              vector<double> next_y_vals;

              // Start with all of the previous path points from las time
              ...
              double x_add_on = 0;// we start at the origin in local coordinates
              ...
              for (int i = 1; i <= 50 - previous_path_x.size(); i++)
              {
                ...
                //go back to global coordinates (rotation + shift):
